...
function dsLogging(appServer) {
	...
	this.logger.notice("Logging started with default log transport");
...
dsLogging.prototype.onConfigChanged_Logging = function(appServer, oldConfig, newConfig) {
	...
	if (typeof newConfig.logging == "undefined") {
		// create a default logger
		this.logger = this.makeLogger({});
		this.logger.notice("Logging successfully reconfigured to use default log transport");
	}
	else {
		// update our logger
		this.logger = this.makeLogger(newConfig.logging);
		this.logger.notice("Logging successfully reconfigured to use configured log transport(s)");
	}
};
