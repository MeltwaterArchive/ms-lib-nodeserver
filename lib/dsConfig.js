...
// our constructor
dsConfig = function(server) {
    // call our parent constructor
    dsConfig.super_.call(this);
    ...
    this.configFiles = [ "app/config.default.json" ];
...
dsConfig.prototype.addConfigFile = function addConfigFile(configFile) {
    ...
    // add a log message
    this.log('added config file ' + configFile + ' to our list to track');
...
dsConfig.prototype.loadAllConfig = function loadAllConfig() {
    ...
    this.log("loading all known config file(s)");
    ...
    // keep the user informed
    this.log("finished loading all known config file(s)");
    ...
    if (success)
    {
        ...
        // tell the server's plugins that the config has changed
        this.server.emit('configChanged', this.server, oldConfig, this.config);
    }
...
dsConfig.prototype.loadConfig = function loadConfig(configFile, config) {
    // does the config file actually exist?
    if (!fs.existsSync(configFile)) {
        this.log('cannot find config file ' + configFile + "; cwd is " + process.cwd());
        return false;
    }

    this.debug('reading config file: ' + configFile);
    ...
    if (data.length === 0)
    {
        ...
        this.debug("config file was empty");
        return;
    }
    ...
    try {
        ...
        this.log("config file " + configFile + " is not valid JSON");
        return false;
    }
...
dsConfig.prototype.updateConfig = function updateConfig() {
    // keep the user informed :)
    this.log("config file(s) has been edited ... reloading");
    ...
    // keep the user informed :)
    this.log("finished reloading config file(s)");
    ...
    if (success)
    {
        var oldConfig = this.config;
        this.config   = newConfig;
        this.server.emit('configChanged', this, oldConfig, this.config);
    }
};
