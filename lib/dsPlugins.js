...
function dsPlugins(appServer, type, loadedList) {
    ...
    appServer.on('configChanged', this.onConfigChanged.bind(this));
}
util.inherits(dsPlugins, dsFeature);
module.exports = dsPlugins;

dsPlugins.prototype.onConfigChanged = function onConfigChanged(appServer, oldConfig, newConfig) {
    ...
    this.addPlugins(appServer, oldConfig, newConfig, newConfig[this.configPathName], newConfig[this.configListName]);
};

dsPlugins.prototype.addPlugins = function addPlugins(appServer, oldConfig, newConfig, pluginsPath, plugins) {
        ...
        if (this.plugins[pluginName] === undefined) {
            ...
            if (this.plugins[pluginName] !== undefined) {
                ...
                if (Object.getPrototypeOf(plugin).onConfigChanged !== undefined) {
                    plugin.onConfigChanged(appServer, oldConfig, newConfig);
                }

                // now tell the world about our new plugin
                this.appServer.emit('pluginInitialised', plugin);
            }
        }
    }
};
