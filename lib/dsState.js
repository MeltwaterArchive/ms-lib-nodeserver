...
// our constructor
function dsState(server) {
    // call our parent constructor
    dsState.super_.call(this, server);
    ...
    server.on('configChanged', this.onConfigChanged_State.bind(this));

    // check the dirty flag periodically
    server.on('every30secs', this.persistState.bind(this));
}
util.inherits(dsState, dsFeature);
module.exports = dsState;

dsState.prototype.onConfigChanged_State = function (server, oldConfig, newConfig) {
    // what has changed?
    ...
    if (oldConfig.stateCacheFile === undefined && newConfig.stateCacheFile !== undefined) {
        ...
        if (server.state.isEmpty()) {
            this.loadState(server.state);
        }
        return;
    }
...
dsState.prototype.persistState = function persistState(server) {
    ...
    var state = JSON.stringify(tagalytics.state);
